...
export class RepositorioEliminarSetProductos {
  /**
   * Elimina un set de productos desde la API
   * @param {array} idsSetProductos - ID del set de productos a eliminar
   * @returns {Promise<{mensaje: string}>}
   */
  static async eliminarSetProductos(idsSetProductos) {
    try {
      const respuesta = await axios.post(
        RUTAS_API.SETS_PRODUCTOS.ELIMINAR_SET_PRODUCTOS,
        { idsSetProductos },
        {
          headers: {
            'x-api-key': API_KEY,
